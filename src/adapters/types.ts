/**
 * The adapter contract. An adapter is the only turnbell component that knows a
 * specific agent's shape: it {@link Adapter.normalize | normalizes} that agent's
 * hook payload into a {@link TurnbellEvent}, and {@link Adapter.wire | wires} the
 * agent's own config so it calls `turnbell emit`. Adapters never talk to sinks —
...
/** Result of probing whether an agent is installed and turnbell-compatible. */
...
export interface AdapterCapabilities {
  ...
  /** Whether turnbell can inject a read-aloud instruction into the agent's global memory file. */
...
export interface Adapter {
   ...
   * is true; turnbell injects the read-aloud block here.
   */
