/**
 * The gemini adapter — wires Google's open-source `gemini` CLI to turnbell.
 *
 ...
 *      shell command's output and `{{args}}` injects user arguments. turnbell writes
 *      `turnbell.toml` so `/turnbell <args>` runs the turnbell CLI from inside Gemini.
 *
 ...
 * turnbell does not parse it — a documented field beats a speculative parser.
 *
 ...
 * the command file turnbell created — so edits a user makes after wiring survive.
 */
...
const COMMAND_FILE = "turnbell.toml";
...
/**
 * The `/turnbell` custom command turnbell owns. `!{...}` runs the shell command and
 * embeds its stdout; `{{args}}` is shell-escaped inside the block. Managed by
 * turnbell and fully reversible via `turnbell uninstall`.
 */
const COMMAND_TEMPLATE = `# Managed by turnbell — reversible via \`turnbell uninstall\`. Do not edit by hand.
...
function addHooks(json: JsonObject): JsonObject {
  ...
  return {
    ...
    hooks: {
      ...hooks,
      [HOOK_AFTER_AGENT]: appendTurnbellHook(hooks[HOOK_AFTER_AGENT], DONE_COMMAND),
      [HOOK_NOTIFICATION]: appendTurnbellHook(
...
/** Strip turnbell's AfterAgent/Notification hook entries, preserving everything else. */
function removeHooks(json: JsonObject): JsonObject {
  return removeTurnbellHooks(json, [HOOK_AFTER_AGENT, HOOK_NOTIFICATION], isTurnbellEntry);
}
